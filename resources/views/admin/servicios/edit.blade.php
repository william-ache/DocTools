...
<div class="max-w-3xl mx-auto space-y-10">
    <div class="fade-element">
        <h1 class="text-4xl font-extrabold text-primary tracking-tighter mb-2">Editar servicio</h1>
        <p class="text-on-surface-variant font-medium">Actualiza la información de este procedimiento médico.</p>
    </div>

    <form action="{{ route('servicios.update', $servicio) }}" method="POST" class="bg-white p-10 rounded-[2.5rem] ambient-shadow border border-surface-container space-y-8">
        @csrf
        @method('PUT')
        
        <div>
            <label class="block text-xs font-black text-on-surface mb-3 tracking-widest uppercase ml-1">Nombre del Servicio *</label>
            <input type="text" name="name" required value="{{ $servicio->name }}" class="block w-full px-6 py-4 border border-surface-container rounded-2xl focus:ring-4 focus:ring-primary/10 transition-all font-bold text-lg">
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-end">
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="number" name="price" required value="{{ $servicio->price }}" class="block w-full pl-12 pr-6 py-4 border-2 border-primary rounded-2xl focus:ring-4 focus:ring-primary/10 transition-all font-black text-xl text-primary">
                </div>
            </div>
            <div class="pb-2">
                <label class="flex items-center gap-4 cursor-pointer group p-4 rounded-xl border-2 border-surface-container has-[:checked]:border-primary transition-all">
                    ...
                    <input type="checkbox" name="price_from" value="1" {{ $servicio->price_from ? 'checked' : '' }} class="w-6 h-6 rounded-lg text-primary focus:ring-primary border-surface-container-highest">
                </label>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
                ...
                <div class="flex items-center gap-4 border border-surface-container rounded-2xl px-4 py-2">
                    <input type="color" name="color" value="{{ $servicio->color ?? '#00478d' }}" class="w-12 h-12 rounded-xl border-none p-0 cursor-pointer overflow-hidden shadow-inner">
                ...
                <div class="relative">
                    <select name="duration" class="block w-full px-6 py-4 border border-surface-container rounded-2xl appearance-none font-bold text-on-surface-variant focus:ring-4 focus:ring-primary/10 transition-all">
                        @foreach([15, 30, 45, 60, 90, 120] as $dur)
                            <option value="{{ $dur }}" {{ $servicio->duration == $dur ? 'selected' : '' }}>{{ $dur >= 60 ? ($dur/60).' Hora(s)' : $dur.' Minutos' }}</option>
                        @endforeach
                    </select>
        ...
        <div>
            ...
            <div class="grid grid-cols-4 sm:grid-cols-6 gap-4">
                ...
                <label class="cursor-pointer group">
                    <input type="radio" name="icon" value="{{ $iconClass }}" {{ $servicio->icon == $iconClass ? 'checked' : '' }} class="hidden peer">
        ...
        <div>
            ...
            <textarea name="description" rows="4" class="block w-full px-6 py-4 border border-surface-container rounded-2xl focus:ring-4 focus:ring-primary/10 transition-all font-medium resize-none shadow-inner">{{ $servicio->description }}</textarea>
        </div>
        ...
        <div class="flex flex-col sm:flex-row gap-6 pt-6 tracking-wide">
            ...
            <button type="submit" class="flex-[2] hero-gradient text-white py-5 rounded-2xl font-black text-lg shadow-xl hover:scale-[1.02] transition-all flex items-center justify-center gap-3">
                <i class="fa-solid fa-sync"></i> Actualizar servicio
            </button>
        </div>
    </form>
    
    <div class="pt-10 border-t border-surface-container">
        <form action="{{ route('servicios.destroy', $servicio) }}" method="POST" class="confirm-delete">
            @csrf
            @method('DELETE')
